...
import 'package:emart_customer/model/variant_info.dart';
...
import 'package:emart_customer/ui/deliveryAddressScreen/DeliveryAddressScreen.dart';
import 'package:emart_customer/ui/productDetailsScreen/ProductDetailsScreen.dart';
import 'package:emart_customer/ui/vendorProductsScreen/NewVendorProductsScreen.dart';
...
class _CartScreenState extends State<CartScreen> {
  ...
  Future<void> getDeliveyData() async {
    ...
    if (selctedOrderTypeValue == "Delivery") {
      if (widget.fromStoreSelection && MyAppState.selectedPosition.latitude == 0.00 && MyAppState.selectedPosition.longitude == 0.00) {
        if (MyAppState.currentUser != null &&
          ...
          num km = num.parse(getKm(Position.fromMap({'latitude': MyAppState.currentUser!.shippingAddress.location.latitude, 'longitude': MyAppState.currentUser!.shippingAddress.location.longitude}),
              Position.fromMap({'latitude': vendorModel!.latitude, 'longitude': vendorModel!.longitude})));
          getDeliveryCharges(km);
        }
      } else {
        num km = num.parse(getKm(MyAppState.selectedPosition, Position.fromMap({'latitude': vendorModel!.latitude, 'longitude': vendorModel!.longitude})));
        getDeliveryCharges(km);
      }
    }
  }
                      ...
                      onTap: () {
                        ...
                        if (productModel!.itemAttributes != null) {
                          if (productModel!.itemAttributes!.variants!.where((element) => element.variant_sku == variantInfo!.variant_sku).isNotEmpty) {
                            if (int.parse(productModel!.itemAttributes!.variants!.where((element) => element.variant_sku == variantInfo!.variant_sku).first.variant_quantity.toString()) > quen ||
                                int.parse(productModel!.itemAttributes!.variants!.where((element) => element.variant_sku == variantInfo!.variant_sku).first.variant_quantity.toString()) == -1) {
                              quen++;
                ...
                : Padding(
                    ...
                    child: Wrap(
                      ...
                      children: List.generate(
                        ...
                        (i) {
                          return _buildChip("${variantInfo!.variant_options!.keys.elementAt(i)} : ${variantInfo.variant_options![variantInfo.variant_options!.keys.elementAt(i)]}", i);
                        },
                      ).toList(),
                    ),
                  ),
          ...
          child: Column(
            children: [
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text(
                        ...
                        style: TextStyle(color: isDarkMode(context) ? const Color(0xffFFFFFF) : const Color(0xff333333), fontSize: selctedOrderTypeValue == "Delivery" ? _font : 15),
                      ),
                    ],
                  )),
              const Divider(
              ...
              selctedOrderTypeValue == "Delivery"
                  ? (widget.fromStoreSelection && !deliverExec! && MyAppState.selectedPosition.latitude == 0.0 && MyAppState.selectedPosition.longitude == 0)
                      ? Padding(
                padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 20),
              ...
              taxModel != null
                  ? Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          Text(
                            ((taxModel!.tax_lable!.isNotEmpty) ? taxModel!.tax_lable.toString() : "Tax") + " ${(taxModel!.tax_type == "fix") ? "" : "(${taxModel!.tax_amount} %)"}",
                            style: TextStyle(fontSize: _font),
                          ),
                          Text(
