...
class CartParcelScreen extends StatefulWidget {
  ...
  CartParcelScreen({Key? key, required this.parcelOrder, required this.images}) : super(key: key);
...
class _CartParcelScreenState extends State<CartParcelScreen> {
  ...
  void initState() {
    ...
    coupon = _fireStoreUtils.getParcelCoupan() as Future<List<OfferModel>>;
  }
  ...
  placeParcelOrder() async {
    ...
    parcelOrderModel.paymentMethod = paymentCollectByReceiverString == "Receiver" ? "cod" : paymentType;
          ...
          onPressed: () async {
            if (paymentCollectByReceiverString == "Sender") {
              ...
              if (razorPay) {
                ...
                paymentType = 'stripe';
                showLoadingAlert();
                stripeMakePayment(amount: getTotalAmount().toStringAsFixed(decimal));
              } else if (payFast) {
                paymentType = 'payfast';
                showLoadingAlert();
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Text("valid till " + getDate(snapshot[index].expireOfferDate!.toDate().toString())!, style: const TextStyle(letterSpacing: 0.5))
                ],
              ),
            ],
          ),
