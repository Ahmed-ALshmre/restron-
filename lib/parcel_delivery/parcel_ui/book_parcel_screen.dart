...
class _BookOrderScreenState extends State<BookParcelScreen> {
  ...
  getParcelWidget() async {
    await FireStoreUtils().getParcelWeight().then((value) {
      if (value != null) {
        setState(() {
          parcelWeight = value as List<ParcelWeightModel>;
        });
                        ...
                        children: [
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () async {
                                ...
                                setState(() {
                                  senderAddress = "${result.subLocalityLevel1!.name == null ? "" : result.subLocalityLevel1!.name.toString() + ", "}${result.locality!.toString()}";
                                  senderLocation = UserLocation(latitude: result.latLng!.latitude, longitude: result.latLng!.longitude);
                                });
                              })
                        ],
                      ),
                        ...
                        children: [
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () async {
                                LocationResult result = await Navigator.of(context).push(MaterialPageRoute(builder: (context) => PlacePicker(GOOGLE_API_KEY)));
                                setState(() {
                                  receiverAddress = "${result.subLocalityLevel1!.name == null ? "" : result.subLocalityLevel1!.name.toString() + ", "}${result.locality!.toString()}";
                                  receiverLocation = UserLocation(latitude: result.latLng!.latitude, longitude: result.latLng!.longitude);
                                });
                              })
                        ],
                      ),
            ...
            if (_key.currentState!.validate()) {
              if (selectedIndex == 1) {
                if (senderData.isEmpty || senderTime.isEmpty) {
                  ...
                  await getDurationDistance(LatLng(senderLocation!.latitude, senderLocation!.longitude), LatLng(receiverLocation!.latitude, receiverLocation!.longitude)).then((durationValue) async {
  ...
  bookParcelOrder() {
    if (distance! >= 1) {
      ParcelOrderModel createOrderModel = ParcelOrderModel(
          ...
          senderPickupDateTime: Timestamp.fromDate(DateTime(selectedDatePickUp.year, selectedDatePickUp.month, selectedDatePickUp.day, selectedTimePickUp.hour, selectedTimePickUp.minute)),
          receiverPickupDateTime: Timestamp.fromDate(DateTime(selectedDateDeliver.year, selectedDateDeliver.month, selectedDateDeliver.day, selectedTimeDeliver.hour, selectedTimeDeliver.minute)),
